    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      def metric_fn(per_example_loss, label_ids, logits, is_real_example):
        ...
        recall = tf.metrics.recall(
            labels=label_ids, predictions=predictions, weights=is_real_example)
        f1 = (2 * recall[0] * precision[0])/(recall[0] + precision[0])
        ...
        return {
            ...
            "eval_recall": recall,
            "eval_f1": (f1, tf.group(precision[1], recall[1]))
        }
...
# 自己新加入
class NcovProcessor(object):
    def get_train_examples(self, data_dir):
        with open(os.path.join(data_dir, "processed_train.txt"), "r", encoding="utf-8") as f:
            lines = f.readlines()
        return self._create_examples(lines, "train")

    def get_dev_examples(self, data_dir):
        with open(os.path.join(data_dir, "processed_dev.txt"), "r", encoding="utf-8") as f:
            lines = f.readlines()
        return self._create_examples(lines, "dev")

    def get_test_examples(self, data_dir):
        with open(os.path.join(data_dir, "processed_test.txt"), "r", encoding="utf-8") as f:
            lines = f.readlines()
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            # 训练集
            if set_type == "train":
                my_label = (line.split(" ")[0]).split("__")[2]
                my_text = " ".join(line.split(" ")[1:]).strip()
                text_a = tokenization.convert_to_unicode(my_text)
                label = tokenization.convert_to_unicode(my_label)
            # 验证集
            if set_type == "dev":
                my_label = (line.split(" ")[0]).split("__")[2]
                my_text = " ".join(line.split(" ")[1:]).strip()
            ...
            if set_type == "test":
                id = line.split("__")[0]
                my_text = " ".join(line.split("__")[1:]).strip()
                text_a = tokenization.convert_to_unicode(my_text)
                label = "0"

            # 这一步的操作是将example封装成一个对象
            examples.append(
